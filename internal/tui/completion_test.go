...
func TestCompletionScreen_Configure(t *testing.T) {
	...
	cs.Configure("auth", 8, 10, "melliza/auth", 5, true, 0, nil)

	if cs.PRDName() != "auth" {
		t.Errorf("expected prdName 'auth', got '%s'", cs.PRDName())
	}
	if cs.Branch() != "melliza/auth" {
		t.Errorf("expected branch 'melliza/auth', got '%s'", cs.Branch())
	}
	if !cs.HasBranch() {
...
func TestCompletionScreen_RenderBranchInfo(t *testing.T) {
	cs := NewCompletionScreen()
	cs.Configure("auth", 8, 8, "melliza/auth", 5, true, 0, nil)
	cs.SetSize(80, 40)

	rendered := cs.Render()
	if !strings.Contains(rendered, "melliza/auth") {
		t.Error("expected branch 'melliza/auth' in render output")
	}
...
func TestCompletionScreen_RenderSingleCommit(t *testing.T) {
	cs := NewCompletionScreen()
	cs.Configure("auth", 1, 1, "melliza/auth", 1, false, 0, nil)
	cs.SetSize(80, 40)
...
func TestCompletionScreen_RenderNoAutoActions(t *testing.T) {
	cs := NewCompletionScreen()
	cs.Configure("auth", 8, 8, "melliza/auth", 5, false, 0, nil)
	cs.SetSize(80, 40)
...
func TestCompletionScreen_ConfigureResetsAutoActions(t *testing.T) {
	...
	cs.Configure("payments", 3, 5, "melliza/payments", 2, false, 0, nil)
